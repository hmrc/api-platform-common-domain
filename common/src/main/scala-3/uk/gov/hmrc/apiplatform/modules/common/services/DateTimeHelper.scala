...
object DateTimeHelper {
  ...
  extension (me: LocalDateTime) {
    ...
    def asInstant: Instant = me.toInstant(ZoneOffset.UTC).truncate()
  }
  ...
  extension (me: Instant) {
    ...
    def asLocalDateTime: LocalDateTime = LocalDateTime.ofInstant(me, ZoneOffset.UTC).truncate()

    /** Converts an Instant to a LocalDate, assuming "local" is in the UTC time zone. Note: truncates the time */
    def asLocalDate: LocalDate = LocalDate.ofInstant(me, ZoneOffset.UTC)
  }
