...
case class ApiIdentifier(context: ApiContext, versionNbr: ApiVersionNbr) {
  def asText(separator: String): String = s"${context}$separator${versionNbr}"
}
...
object ApiIdentifier {
  ...
  )(i => (i.context, i.versionNbr))

  given OFormat[ApiIdentifier] = OFormat[ApiIdentifier](readsApiIdentifier, writesApiIdentifier)

  given Ordering[ApiIdentifier] = Ordering.by[ApiIdentifier, ApiContext](_.context).orElseBy[ApiVersionNbr](_.versionNbr)
